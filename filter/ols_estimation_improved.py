...
class OLSEstimator:
    ...
    def generate_trading_signals_with_fallback(self, spread_data: pd.DataFrame,
                                            entry_threshold: float = 1.0,
                                            exit_threshold: float = 0.5,
                                            correlation_threshold: float = 0.4) -> pd.DataFrame:
        """
        Generate trading signals with fallback strategy when cointegration fails
        
        Args:
            spread_data (pd.DataFrame): Spread analysis results
            entry_threshold (float): Entry threshold for z-score (reduced from 2.0)
            exit_threshold (float): Exit threshold for z-score
            correlation_threshold (float): Minimum correlation threshold (reduced from 0.6)
    ...
    def calculate_hedge_ratio_with_fallback(self, y: pd.Series, x: pd.Series, 
                                          method: str = 'ols',
                                          correlation_threshold: float = 0.4) -> Dict:
        """
        Calculate hedge ratio with fallback when cointegration fails
        
        Args:
            y (pd.Series): Dependent variable (futures)
            x (pd.Series): Independent variable (stock)
            method (str): Estimation method
            correlation_threshold (float): Minimum correlation threshold
            
        Returns:
            Dict: Hedge ratio results with fallback
        """
        # First try standard OLS
        try:
            result = self.calculate_hedge_ratio(y, x, method)
            
            # If cointegration fails but correlation is sufficient, use fallback
            if not result['is_cointegrated']:
                correlation = y.corr(x)
                if abs(correlation) >= correlation_threshold:
                    print(f"Cointegration failed but correlation sufficient ({correlation:.3f}), using fallback")
            ...
            # Complete fallback - use simple correlation-based hedge ratio
            correlation = y.corr(x)
            if abs(correlation) >= correlation_threshold:
                print(f"OLS failed, using correlation-based fallback ({correlation:.3f})")
                return {
                    'hedge_ratio': correlation,
                    'r_squared': correlation**2,
                    'p_value': 0.05,  # Assume significance
                ...
                return {
                    ...
                    'use_fallback': False,
                    'correlation': correlation,
                    'error': f"Correlation too low: {correlation:.3f} < {correlation_threshold}"
                }
